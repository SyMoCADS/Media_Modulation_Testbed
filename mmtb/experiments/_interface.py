...
class ExperimentSelectionWindow:
    def __init__(self, prev_selected_exp: Optional[list[str]] = None):
        ...
        if prev_selected_exp is not None and len(prev_selected_exp) > 0:
            ...
            for exp in prev_selected_exp:
                self._selected_list.insert(tk.END, exp)
                self._selected_list.select_set(self._selected_list.size() - 1)
                self._selected_list.event_generate("<<ListboxSelect>>")
    ...
    def _show_exp_info(self, event, id: str):
        ...
        if selection:
            ...
            experiment_info = self._database.get_info(experiment_name=experiment_name, date=date)

            if len(experiment_info) == 0:
                self._remove_selected_exp()
                return

            experiment_info = experiment_info[0]
    ...
    def _add_exp_to_selection(self, event) -> None:
        try:
            ...
            raise RuntimeError("The experiment info could not be displayed, since either the file does not exist anymore or has the wrong format.")
    ...
    def _remove_selected_exp(self, event=None) -> None:
        selection = self._selected_list.curselection()
