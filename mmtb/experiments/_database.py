...
class ExperimentDatabaseMeta(type):
    ...
    def __call__(cls):
        if cls._instance is None:
            cwd_path = os.getcwd()
            database_path = None
            default_database_path = os.path.join(Path(__file__).parent, "mmtb.db")
            default_database_size = os.path.getsize(default_database_path)

            for root, _, files in os.walk(cwd_path, topdown=True):
                if "mmtb.db" not in files:
                    continue
                
                tmp_database_path = os.path.join(root, "mmtb.db")
                
                if os.path.getsize(tmp_database_path) <= default_database_size:
                    continue 
                
                database_path = tmp_database_path 

            if database_path is None:
                database_path = os.path.abspath(default_database_path)
