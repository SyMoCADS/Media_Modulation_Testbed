...
if __name__ == "__main__":
    ...
    #experiment_data = get_selected_experiments(select_files=['(2024-08-21) TX6_Ts5s_Mod2_wEX']) # Direct experiment data acces without selection window
    ...
    for experiment in experiment_data:
        ...
        print("Averaged normalized absolute synchronization error: {}".format(mmtb.evaluation.calculate_avg_norm_abs_rel_sync_error(t_symbol=experiment.trans_param.t_symbol,
                                                                    sync_data=synchronization_data)))

        # Computes the bit error rate using Gray mapping and prints the result to stdout.
        print("Bit error rate (Gray mapping): {}".format(mmtb.evaluation.calculate_bit_error_rate_gray_mapping(n_pilots=n_pilots,
        ...
        if len(experiment.spec_settings.eval_wavelengths) == 1:
            ax1.set_title("Normalized Fluorescence at {} nm".format(experiment.spec_settings.eval_wavelengths[0]))
        else:
            ax1.set_title("Normalized Averaged Fluorescence between {} nm to {} nm".format(experiment.spec_settings.eval_wavelengths[0], experiment.spec_settings.eval_wavelengths[1]))
        ax1.set_ylim(0, 1.2)
